...
public:
    vector<vector<string>> deleteDuplicateFolder(vector<vector<string>>& paths) {
        ...
        vheight[1].push_back({});
        map<string,set<Node*>> &ms=vheight[1].front();
        map<string,set<Node*>> msleaf;
        dfsleaf(root,msleaf);


        for(auto ite = msleaf.begin();ite!=msleaf.end();ite++){
            if(ite->second.size() < 2){continue;}
            int sid = ++ sameid;
            ms.insert({ite->first,{}});
            set<Node*> &s = ms[ite->first];
            for(auto ite2 = ite->second.begin();ite2!=ite->second.end();ite2++){
                (*ite2)->issame = sid;
                if(s.find((*ite2)->parent) == s.end()){
                    s.insert((*ite2)->parent);
                }
            }
        }
