    ...
    double minRecSize(vector<vector<int>>& lines) {
        ...
        for(int i = 0; i < vvn-1;i++){  
            ...
            for(int j = i+2; j < vvn; j ++){
                ...
                jminb = v[jk].first;
                jmaxb = v[jk].second;
                if((jmaxb - iminb) * (ik - mink) < (v[mink].second - iminb) * (ik - jk)){
                    mink = jk;
                }
                if((jminb - imaxb) * (ik - mink) > (v[maxk].first - imaxb) * (ik - jk)){
                    maxk = jk;
                }
            }

            minx = 1.0 * (v[mink].second - iminb) / (ik - mink);
            maxx = 1.0*(v[maxk].first - imaxb) / (ik - maxk);
            miny = ik * minx + iminb;
            maxy = ik * maxx + imaxb;
