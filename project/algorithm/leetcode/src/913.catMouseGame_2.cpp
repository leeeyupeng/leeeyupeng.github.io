...
private:
	...
	int minimax(vector<vector<int>>& graph,int lastmouse,int lastcat,int mouse, int cat,bool mouseplay) {		
		if (m[mouseplay?0:1][mouse][cat] != -10) {
		...
		if (mouseplay) {
			...
			for (int i = 0; i < graph[mouse].size(); i++) {
				...
				else if (us.find(mouse * MAX * MAX * MAX + lastcat * MAX * MAX + graph[mouse][i] * MAX + cat) != us.end()) {
					res = max(res, 0);
					//res = res == 1 ? res : 0;
				}
				else {
					us.insert(mouse * MAX * MAX * MAX + lastcat * MAX * MAX + graph[mouse][i] * MAX + cat);
			...
			for (int j = 0; j < graph[cat].size(); j++) {
				...
				else if (us.find(lastmouse * MAX * MAX * MAX + cat * MAX * MAX + mouse * MAX + graph[cat][j]) != us.end()) {
					res = min(res, 0);
					//res = res == 1 ? res : 0;
				}
				else 
				{ 
					us.insert(lastmouse * MAX * MAX * MAX + cat * MAX * MAX + mouse * MAX + graph[cat][j]);
