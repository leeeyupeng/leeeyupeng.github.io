...
private:
	...
	struct bb {
		...
		bb(int _index, int _v, int _cost, int _eval,int _nums) :index(_index), v(_v), cost(_cost), eval(_eval),nums(_nums) {}
		bool operator <=(bb& other) const
		{
			if (eval + cost == other.cost + other.eval)
			{
	...
	int BranchBound()
	{
		...
		for (int i = 0; i < mnums; i++) {
			if (StoOtoM[i] != INT_MAX) {
				q.Push(bb(i, 1 << i, StoOtoM[i], eval(i, 1 << i),1));
			}
		}
		while (!q.Empty()) {
			bb top = q.Top();
				...
				if (((top.v >> i) & 0x1) == 0x0 && MtoOtoM[top.index][i] != INT_MAX) {
					...
					if (top.cost + MtoOtoM[top.index][i] + tempeval < ans) {
						if (map.find(key2) != map.end() && map[key2] <= top.cost + MtoOtoM[top.index][i]) {
							...
							q.Push(bb(i, top.v + (1 << i), top.cost + MtoOtoM[top.index][i], tempeval,top.nums + 1));
						}
					}
				}
			}
