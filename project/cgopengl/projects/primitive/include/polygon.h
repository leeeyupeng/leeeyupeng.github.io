...
int main_pro()
{
	...
	int vnums = sizeof(vertices) / 6 / sizeof(float);
	vector<unsigned int> vindice;
	for (int i = 0; i < vnums - 2; i++) {
		vindice.push_back(0);
		vindice.push_back(1+i);
		vindice.push_back(2+i);
	}
	unsigned int * indices;
	indices =(unsigned int *) malloc((vnums - 2) * sizeof(unsigned int));
	...
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, (vnums - 2) * 3 * sizeof(unsigned int), &vindice[0], GL_STATIC_DRAW);
	//glBufferData(GL_ELEMENT_ARRAY_BUFFER, (vnums - 2) * 3 * sizeof(unsigned int), indices, GL_STATIC_DRAW);
	// position attribute
