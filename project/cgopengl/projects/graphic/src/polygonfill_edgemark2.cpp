...
#include"polygonfill_edgemark2.hpp"
using namespace std;

inline void minpointlinefill2(int n, int m, bool* mark, lt::Vector3<float>* markcolor, lt::Vector2<int> pointa, lt::Vector2<int> pointb) {
	int x = pointa.x, y = pointa.y;
	mark[x*m + y] = !mark[x*m + y];
	...
	else
	{
		if (a <= 0 && b >= 0) {
			...
			while (y < pointb.y) {
				if (d > 0) {
					x += dx; d += d2; mark[x*m + y] = !mark[x*m + y];
				}
				else { d += d1; }
				y += dy;
			}
...
lt::polygonfill_edgemark2::polygonfill_edgemark2(int _size, lt::Vector2<int>* _verticle, lt::Vector3<float>* _color) {
	size = _size;
	verticle = _verticle;
	color = _color;
};
lt::polygonfill_edgemark2::~polygonfill_edgemark2() {
};
void lt::polygonfill_edgemark2::Init() {
	...
	for (int i = 0; i < size; i++) {
		...
		minpointlinefill2(n,m,mark,markcolor, verticle[(size + i - 1) % size] - lt::Vector2<int>(minx,miny), verticle[i] - lt::Vector2<int>(minx, miny));
	}
	...
	for (int i = 0; i < size; i++) {
		lt::Vector2<int> a = verticle[i] - lt::Vector2<int>(minx, miny);
		mark[a.x * m + a.y] = false;
	}
...
void lt::polygonfill_edgemark2::Destroy() {
	glDeleteVertexArrays(1, &VAO);
	glDeleteBuffers(1, &VBO);
}
void lt::polygonfill_edgemark2::Display() {
	glPointSize(1.0f);
	glBindVertexArray(VAO);
