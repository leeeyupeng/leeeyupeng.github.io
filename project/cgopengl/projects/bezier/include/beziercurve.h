...
int main_pro()
{
    ...
    // --------------------
    GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "beziercurve", NULL, NULL);
    if (window == NULL)
    {
	...
	float vertices[NUMPOINT * 6];
	for (int i = 0; i < NUMPOINT; i++) {
		float t = i * 1.0f / (NUMPOINT-1);
		for (int j = 0; j < 6; j++) {
			vertices[i * 6 + j] = vertices_p[0*6+j]*pow(1.0f-t,3);
			vertices[i * 6 + j] += 3 * vertices_p[1 * 6 + j] * t * pow(1.0f - t, 2);
			vertices[i * 6 + j] += 3 * vertices_p[2 * 6 + j] * pow(t,2) * (1.0f - t);
			vertices[i * 6 + j] += vertices_p[3 * 6 + j] * pow(t, 3);
		}
	}
