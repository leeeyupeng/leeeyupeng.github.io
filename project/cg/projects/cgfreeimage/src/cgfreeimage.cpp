...
WCHAR* szTitle = TEXT("cgfreeimage");                  // 标题栏文本
...
int main()
{
	...
	*/
	...
	const char imgName[512] = "resources/icon512.png";
	FREE_IMAGE_FORMAT fifmt = FreeImage_GetFileType(imgName,0);
	// 加载此文件
	dib = FreeImage_Load(fifmt, imgName, 0);
	if (dib != NULL) {
		// 为了省事
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_PAINT:
	{
		...
		StretchDIBits(hdc, 0, 0, width, height, 0, 0, width, height, pixels, &info, DIB_RGB_COLORS, SRCCOPY);
